 ...
 *
 * ## CP(eaog)
 * Cognitive Program 是一种可执行的树结构(Executable And Or Graph），其叶子节点是 Action。非叶节点是结构��点，这些节点说明孩子节点的执行顺序（顺序、并行）与关系（条件）。
...
const getNodeTypeConfig = (type: string) => {
  return nodeTypeUIConfig[type as keyof typeof nodeTypeUIConfig] || nodeTypeUIConfig['_default'];
};
