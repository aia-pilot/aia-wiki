...
const dynamicObjectSchema = z.object({
  name: z.string().min(1).describe('动态对象名称'),
  value: z.record(z.string(), z.union([z.string(), z.function(), z.any()])).describe('对象的键值对，可在表单中动态添加和删除'),
});
...
const [Form, formApi] = useVbenForm(reactive({
  ...
  wrapperClass: 'grid-cols-4', // 设置容器为4列布局，提供更细粒度的控制
...
const handleSubmit = async () => {
  ...
  if (valid) {
    ...
    // 验证数据
    const result = dynamicObjectSchema.safeParse(transformedValues);
    if (result.success) {
